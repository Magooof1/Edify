...
Dated log of compliance issues Ed has flagged in past drafts/concepts and how they were
resolved (or where they stand if unresolved). Purpose: don't repeat the same mistake or
re-litigate a call already made. Ed flags; Justin (and sometimes outside counsel)
decides — this log records outcomes, it doesn't make them.

---

## 2026-08-25 — Client-appreciation gift/event concepts (family photo shoot; client-funded retirement celebration)

**What happened:** Ed reviewed two client-appreciation event concepts — (1) a free family
photo shoot given to a small group of clients, and (2) a client-funded retirement
celebration where the client invites coworkers. Verdict: not a clean pass.

**Why this one matters beyond the two concepts:** first time the workforce reviewed an
in-kind GIFT/EVENT concept rather than a content piece (script, post, email). Gifts and
events trigger a materially different rule set than content does — see checklist below
for future reference.

**Precedent — checklist for any future gift/event idea (not just written/recorded
content):**
1. **State insurance anti-rebating law** (Ohio ORC 3901.21; Kentucky has its own separate
   statute) — applies if any recipient has insurance/annuity business with Trulip. Gifts
   of value tied to that business are capped by state-set dollar limits. Confirm the
   current cap at review time — don't assume a previously-cited number still holds.
2. **SEC Marketing Rule promoter/testimonial provisions** — applies if the event involves
   a client facilitating introductions to other people, even informally, even with a
   stated "no pitch" intent. Funding something a client uses to bring prospects into the
   advisor's orbit can read as non-cash compensation to a promoter. This is a genuinely
...
- Outside counsel's read on whether the retirement-celebration structure trips promoter
