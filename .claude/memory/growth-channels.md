# Growth Channels & Marketing Budget
...
(a) institutional/catalog promotion (e.g., a college's own class catalog) and (b)
targeted direct-mail invitations to attend. The mailer is a top-of-funnel tactic that
feeds the class, not a standalone channel with its own independent conversion path.
Practical consequence: the mailer's declining unit output and the classes' declining
attendance are **two measurements of the same underlying saturation**, not two
independent signals that could be softening for unrelated reasons. Do not analyze or
report on them as if they were separable channels.

## Class-based lead generation (primary channel, historical)
- Justin's primary lead-gen channel from founding through ~5 years to $22M AUM.
- Filled via two feeder tactics: institutional/catalog promotion and targeted direct-
  mail invitations (see "Direct mail as a feeder tactic" below). Both feed the same
  class attendance funnel.
- 2026-08-25: Justin flagged this channel as "starting to saturate" (his word).
- Attendance decline, confirmed (2026-08-25): over the last 2 years, some individual
  classes have drawn as few as 2 buying units (households) — a real per-class decline.
...
- Strategic implication (diagnosis only, not a decision — flagged for Taleb's
  ...
  and employers as venue types alongside colleges, but Justin's actual channel
  history has been almost entirely college classes (NKU) — that diversification may
  be underused.

### Direct mail as a feeder tactic (not a separate channel)
...
- **Final data on the specific $10K mailer (confirmed 2026-08-25):** produced 3
  buying units total — well below the historical 10-20 unit range (roughly a 70%+
  shortfall from even the low end).
  - Unit 1: no-go, dead lead.
  - Unit 2: pending, roughly a $1M prospect. Justin is uncertain whether this closes
    — the prospect is risk-averse. There is a real chance of no-close, not just an
    administrative delay.
  - Unit 3: closed. A pension was rolled over into an annuity, generating
    approximately $6,000 in commission revenue. Confirmed as commission revenue, not
    AUM/fee revenue — this deal type does not produce ongoing AUM the way a managed-
    account placement would, so it is not comparable to, and should not be used to
    estimate, an AUM figure.
- **ROI status: unresolved, binary/high-variance — not a success or failure call.**
  This mailer's real return currently hinges almost entirely on whether the ~$1M
  pending prospect (Unit 2) closes. If it closes, the mailer produced one real AUM
...
- New tactical ideas (e.g., a YouTube channel) are paused while Justin builds an
  overall marketing plan first, given saturation signs in the primary channel.
- This is the reason behind whatever the eventual marketing plan concludes — worth
...
- Does the ~$1M pending prospect (Unit 2 of the $10K mailer) actually close? Live
  open question — Justin himself is uncertain, and the mailer's ROI verdict depends
  on the answer.

Resolved (2026-08-25): why the college-class channel is saturating — see
"Saturation cause" under Class-based lead generation above. Format/competitor
crowding, not demand or execution/skill.

Resolved (2026-08-25): status of the third buying unit from the $10K mailer — no-go,
dead lead (Unit 1 above; note unit numbering in this file is arbitrary bookkeeping,
not a sequence tied to when each was contacted).

Resolved (2026-08-25): rough AUM figure produced by the $10K mailer — none yet. Of
the 3 buying units, one is dead (no AUM), one closed as a commission-only annuity
deal (~$6K commission, not AUM), and one is a pending ~$1M prospect that may or may
not close. No AUM has actually landed from this mailer as of 2026-08-25.
...
## 2026-09-19 (later) — Vendor/creative facts close the design hypothesis; offer-threshold hypothesis raised, then Taleb's verdict (same day)
...
- **The piece improved while results declined — the most important evidence in this
  ...
  below.
- **Terminology fix, apply wherever this number is used.** The ~90% figure is
  attendees who **book a follow-up meeting**, not attendees who become clients —
  see growth-channels.md line ~38 ("~90% of class attendees now book a follow-up
  meeting"). Isla had been saying "closes at 90%" for two days and that looser
  framing propagated into a response-rate model built with Sloan; class-content.md's
  header phrase ("attendees who take the next step become clients") was ambiguous
  enough to read the same way and has been corrected (2026-09-19) to say booking,
  not closing. **Booking rate and close rate are different numbers — don't
  interchange them in any funnel model.**
- **Taleb's verdict, landed 2026-09-19: half right, and not to be recorded as an
  explanation of the 90% booking rate.** What survives: a 3.5-hour Saturday
  commitment from a still-working 50-65 population is a real filter that plausibly
  explains why the *addressable pool* for this specific offer is small and finite
  even where list-level proxies (ZIP, IPA, net worth, age) look healthy. Taleb's
  useful distinction: list quality and willingness-to-clear-the-offer's-bar are
  different variables — a household can be strong on the first while used up on the
  second. Reading ZIP strength as audience strength is the error this hides.
  **What fails:** the threshold as an explanation of *why the room converts at
  ~90%*. The 2026-09-12 eval-form data already on record (discovery vs.
  confirmation) shows the split is generated by whether the three-bucket frame
  lands, not by who showed up — non-converters didn't fail to attend or engage,
  they checked Red or taxes instead of Blue. If pre-qualification-by-sacrifice
  explained the rate, it would sit near 100%, since everyone in the room cleared
  the identical bar. **So: a reasonable answer to "why is the room emptying," an
  unsupported answer to "why does the room convert"** — the second question was
  already answered by the eval-form work.
- **Provenance — this is an extension, not a new finding.** Taleb's read: the
  threshold hypothesis refines the already-resolved 2026-08-25 format-crowding
  entry above, not a separate discovery — it was presented as new because the
  hypothesis was formed before that entry was read. **The 2026-08-25 finding
  stands as the primary diagnosis**: format/competitor crowding, corroborated by
  two independent advisors reporting the same attendance decline, explicitly not
  NKU-specific and not a Justin-execution problem. The offer-threshold point is
  filed as an extension of that entry, not alongside it as a co-equal cause.
- **New, unweighed hypothesis — open, untested, not to be treated as dismissed.**
  A secular, industry-wide decline in direct-mail response, independent of
  Justin's offer or geography, explains the same evidence with no new facts about
  his specific market. Isla's own weighting for the record: likely a contributor
  rather than the primary cause — a 3-5x collapse over a few years is steeper than
  typical industry drift — but this hasn't been checked against any published
  benchmark, so log it as untested, not as weighed and rejected.
- **Open, un-run experiment — Taleb's proposal, Justin's call, not yet made.** Run
  one short, low-threshold session (90 minutes, or a trimmed Saturday block) into
  the exact ZIPs Justin calls tired. Attendance jumping from an "exhausted" list
  would indicate the binding constraint is the ask (threshold), not the
